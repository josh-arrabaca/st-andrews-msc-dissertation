...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    with TemporaryDirectory() as tempdir:
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.title("Model Loss over Epochs for momentum=" + str(momentum))
        plt.savefig("losses_"+str(momentum)+".png", bbox_inches='tight')
        plt.close()
...
# Here we try different momentum values with the lr set to the best lr
momentum_values = [0.90, 0.95, 0.97, 0.99]

for value in momentum_values:
    momentum = value
    optimizer_conv = optim.SGD(model_conv.fc.parameters(), lr=0.01, momentum=momentum)

    # Decay LR by a factor of 0.1 every 7 epochs
    exp_lr_scheduler = lr_scheduler.StepLR(optimizer_conv, step_size=7, gamma=0.1)

    # Now train the model and see the loss and accuracy scores
